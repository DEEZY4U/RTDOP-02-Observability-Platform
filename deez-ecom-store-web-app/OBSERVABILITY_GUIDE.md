...
This guide demonstrates **end-to-end visibility** for the Deez Ecom Store purchase journey, showcasing how to follow a user request from browser click to database response and back through all system components.
...
docker logs deez-ecom-store-api | grep "order_id.*7"

# Or search for recent checkout events
docker logs deez-ecom-store-api | grep "checkout" | tail -5
```

#### Step 3: View in Jaeger
1. Visit: `http://localhost:16686`
2. Search service: `deez-ecom-store-api`
3. Search operation: `checkout_order`
4. Look for traces with matching order_id

#### Step 4: Check Dashboard
Visit the "Deez Ecom Store - End-to-End Purchase Journey" dashboard in Grafana.
...
docker logs deez-ecom-store-api | grep "ERROR" | grep "checkout"
...
#### 3. Analyze Traces
- Go to Jaeger UI
- Search by service: `deez-ecom-store-api`
...
1. Stop the database: `docker stop deez-ecom-store-db`
...
5. Restart database: `docker start deez-ecom-store-db`
