...
# Artifact Repository Cleanup Script for Deez Ecom
...
0 2 * * 0 cd /path/to/deez-ecom && ./scripts/cleanup_old_artifacts.sh > /var/log/artifact-cleanup.log 2>&1

# Monthly comprehensive cleanup
# Runs first Sunday of every month at 3 AM UTC
0 3 1-7 * 0 cd /path/to/deez-ecom && VERBOSE=true ./scripts/cleanup_old_artifacts.sh > /var/log/artifact-cleanup-monthly.log 2>&1
EOF
...
# Main execution
main() {
    echo -e "${BLUE}🧹 Deez Ecom - Artifact Repository Cleanup${NC}"
    echo "=============================================="
